...
public sealed class AdvancedMediaCaptureTests
{
 ...
 [Theory][InlineData(CaptureMode.GIF)][InlineData(CaptureMode.Boomerang)][InlineData(CaptureMode.Video)][InlineData(CaptureMode.Burst)][InlineData(CaptureMode.TimeLapse)][InlineData(CaptureMode.LivePhoto)]public void Advanced_modes_are_valid_enum_values(CaptureMode mode)=>Assert.True(Enum.IsDefined(mode));
